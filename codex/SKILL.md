...
version: 1.0.0
description: |
  OpenAI Codex CLI 封装器，支持三种模式。Code review：通过 `codex review`
  对 diff 做独立评审，并给出 pass / fail gate。Challenge：以对抗视角尝试
  找出你的代码会如何在生产环境里失效。Consult：把 Codex 当作一个可持续对话的
  第二意见来源，支持后续追问。适用于用户说 “codex review”、“codex challenge”、
  “ask codex”、“second opinion” 或 “consult codex” 的场景。
allowed-tools:
  - Bash
  - Read
  - Write
...
你正在执行 `/codex` 技能。这个技能会包装 OpenAI Codex CLI，从另一个 AI 系统那里获取独立且不留情面的第二意见。

Codex 的角色设定是：直接、简短、技术精确、会挑战假设，并抓出你可能漏掉的东西。它的输出要尽量忠实呈现，不要擅自总结得面目全非。
...
3. `/codex` 无额外参数 → **自动检测：**
   - 先检查当前是否有 diff（如果 origin 不可用则回退）：
   ...
   - 如果存在 diff，则使用 AskUserQuestion：
     ...
     ```
   - 如果没有 diff，则检查当前项目范围内是否有 plan 文件：
     `ls -t ~/.claude/plans/*.md 2>/dev/null | xargs grep -l "$(basename $(pwd))" 2>/dev/null | head -1`
     如果没有项目级匹配，再回退到：`ls -t ~/.claude/plans/*.md 2>/dev/null | head -1`
     但要提醒用户：`"Note: this plan may be from a different project."`
   - 如果找到 plan 文件，就提供“要不要审这个 plan”的选项
   - 否则直接问：`"What would you like to ask Codex?"`
4. `/codex <anything else>` → **Consult mode**（Step 2C），剩余文本直接作为 prompt
