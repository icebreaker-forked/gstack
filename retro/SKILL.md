...
- 低谷时段
- 模式是双峰（早晚各一波）还是连续分布
- 是否存在明显的深夜编码集中段（晚上 10 点之后）

### Step 4：工作会话识别

使用**45 分钟间隔阈值**把连续 commit 划分成不同 session。对每个 session 都要报告：
- 开始 / 结束时间（使用本地时间）
- commit 数
- 持续时长（分钟）
...
- **Micro sessions**（20 分钟以下，通常是一两个快速提交）

并计算：
- 总活跃编码时长（session 时长总和）
- 平均 session 长度
- 每小时活跃时间内的 LOC 产出
...
如果 fix 占比超过 50%，要明确标出来。这通常意味着“先快速发，再快速补”的模式，背后可能存在 review 缺口。

### Step 6：热点文件分析

展示变更最频繁的前 10 个文件，并标记：
- 被改动 5 次以上的文件（高 churn 热点）
- 热点列表里测试文件与生产文件的比例
- `VERSION` / `CHANGELOG` 出现频率（版本纪律的信号）

### Step 7：PR 规模分布

从 commit diff 粗略估算 PR 体量，并按以下区间分桶：
- **Small**（<100 LOC）
- **Medium**（100-500 LOC）
- **Large**（500-1500 LOC）
- **XL**（1500+ LOC）——这类要额外标出涉及文件数

### Step 8：Focus Score + Ship of the Week

**Focus score：** 计算有多少 commit 触碰了“最常被改动的顶层目录”（例如 `app/services/`、`app/views/`）。分数越高，表示工作越集中；越低，则说明上下文切换更散。输出格式例如：`"Focus score: 62% (app/services/)"`

**Ship of the week：** 自动识别当前时间窗口里 LOC 最大的一次 ship / PR，并重点展示：
- PR 编号和标题
- 变更 LOC
- 为什么它重要（根据 commit message 和变更文件做推断）

### Step 9：团队成员分析

对每个贡献者（包括当前用户），都要计算：

1. **Commits and LOC** —— 总提交数、insertions、deletions、净 LOC
2. **Areas of focus** —— 他们最常改的目录 / 文件（前 3）
3. **Commit type mix** —— 他们个人的 feat / fix / refactor / test 分布
4. **Session patterns** —— 他们通常何时编码（高峰时段）、session 数量
5. **Test discipline** —— 他们个人的测试 LOC 比例
6. **Biggest ship** —— 当前窗口内他们影响最大的一次提交或 PR

**对于当前用户（“You”）：** 这一部分要写得最深入。把单人 retro 中该有的细节都带上，例如 session 分析、时间模式、focus score。叙述上要以 “Your peak hours...” / “Your biggest ship...” 这样的方式组织。

**对于每个队友：** 用 2-3 句话概述他们主要做了什么、呈现出什么工作模式。然后：

- **Praise**（1-2 个具体点）：必须锚定在真实提交上。不要泛泛地说 “great work”，而是明确说清哪里做得好。例如：“在 3 个高度集中的 session 里完成整个 auth middleware 重写，并保持了 45% 测试覆盖”“所有 PR 都控制在 200 LOC 以内，拆分纪律很好。”
